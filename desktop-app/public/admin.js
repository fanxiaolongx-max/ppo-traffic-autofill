...
function renderSmsRules() {
  const english=getLanguage()==='en';
  $('#sms-rules').innerHTML=state.smsRules.map((rule,index)=>`<div class="sms-rule" data-sms-rule="${index}"><input class="sms-rule-enabled" type="checkbox" ${rule.enabled!==false?'checked':''} title="${english?'Enabled':'启用'}"><select class="sms-rule-type"><option value="document" ${rule.type==='document'?'selected':''}>${english?'Document':'证件号'}</option><option value="plate" ${rule.type==='plate'?'selected':''}>${english?'Plate':'车牌号'}</option></select><input class="sms-rule-match" value="${escapeHtml(rule.matchValue||'')}" placeholder="${english?'Exact value':'精确匹配值'}" required><input class="sms-rule-to" value="${escapeHtml(rule.to||'')}" placeholder="${english?'Recipient phone':'接收手机号'}" required><textarea class="sms-rule-template" maxlength="500" placeholder="${english?'Optional message template':'可选短信模板'}">${escapeHtml(rule.template||'')}</textarea><button class="sms-rule-remove" type="button">${english?'Remove':'删除'}</button><input class="sms-rule-id" type="hidden" value="${escapeHtml(rule.id||'')}"></div>`).join('')||`<p class="muted">${english?'No rules. Add one to start matching.':'暂无规则，点击“新增规则”开始配置。'}</p>`;
  document.querySelectorAll('.sms-rule-remove').forEach(button=>button.addEventListener('click',()=>{state.smsRules.splice(Number(button.closest('[data-sms-rule]').dataset.smsRule),1);renderSmsRules();}));
}
...
async function loadSmsDeliveries({append=false}={}) {
  if(!append)state.smsDeliveryCursor=''; const params=new URLSearchParams({limit:'20'}); if(append&&state.smsDeliveryCursor)params.set('cursor',state.smsDeliveryCursor);
  const data=await adminApi(`/api/v1/admin/sms/deliveries?${params}`); state.smsDeliveryCursor=data.nextCursor||''; state.smsDeliveries=append?[...state.smsDeliveries,...data.items]:data.items; renderSmsDeliveries(data,append);
}
