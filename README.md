...
项目包含三个独立版本，版本号不要求始终相同：

| 组件 | 当前版本 | 更新方式 |
| --- | --- | --- |
| Chrome / Edge 浏览器插件 | `1.0.9` | 完整 Release，重新生成商店 ZIP |
| macOS 稳定外壳 App | `1.0.9` | 完整 Release，重新生成 DMG / ZIP |
| 可热更新查询核心 | `1.0.10` | 独立 `core-v1.0.10` Release，只下载签名核心包 |

稳定外壳负责 Electron、托盘、固定端口网关、核心验签与切换；查询核心负责 Web UI、API、队列、数据库访问和 PPO 查询逻辑。因此，只修改查询核心时外壳和浏览器插件仍可保持 `1.0.9`，已安装的 App 会在 Admin 或托盘菜单中发现并平滑切换到核心 `1.0.10`。只有 Electron 外壳、托盘、网关、验签公钥、原生依赖或浏览器插件发生变化时，才需要发布新的完整 App / 插件版本。

这里的“当前版本”是本文更新时的发布状态；后续应以 GitHub 中最新的 `vX.Y.Z` 和 `core-vX.Y.Z` Release 为准。

## 1.0.9 更新内容

- 桌面程序改为稳定外壳网关与版本化查询核心：支持签名核心包下载、队列排空、维护页、双版本切换、健康检查、数据库备份和失败自动回滚。
- 查询、反馈、结构化日志、Admin 服务状态以及公开历史/状态统一采用服务端游标分页，完整数据搜索不再受最近 5000 条限制。
- GitHub 增加独立查询核心发布流程，核心逻辑更新可只发布签名数据包，无需重新构建完整 macOS App。
- 修复 macOS 原生更新提示框进入模态循环后阻塞本地 Web/API 服务的问题；更新结果改为非阻塞系统通知，发现新版本时可点击通知打开 Release 页面。
- 修复 macOS 菜单栏托盘图标在暗色/透明菜单栏中不可见的问题
- 修复反向代理访问被错误记录为 `127.0.0.1`，查询与反馈统一记录可信来源的真实客户端 IP
- 建议反馈支持受限截图与文件附件，Admin 可鉴权预览或下载
- Admin 可按完整证件号或车牌号配置精确匹配的短信通知规则，并查看、新增、修改、暂停或删除用户手机号绑定；短信 API 会启动即检、周期复检，状态同时显示在 Admin 与用户服务状态窗口。查询成功或失败后异步调用外部短信 API，使用固定幂等键防止重复发送，并单独保存投递状态，不阻塞查询队列
- Admin 可对单条已启用绑定手动执行一次排队查询并推送；所有短信配置写操作和测试推送均要求二次确认
- 用户可在查询成功后的结果详情绑定埃及手机号；`+20` 前缀固定，支持输入带或不带本地 `0` 的号码，必须完成短信验证码验证才会生效。验证后默认每 7 天自动查询，可按天调整且最短 1 天，并与前台查询共用串行队列及设备 ID + IP 防滥用限额
- 周期短信以单行显示“本期变化＋当前累计”，例如 `本期 +2笔 / +350 جنيه｜累计 5笔 / 1,200 جنيه`，无需用户记忆上次总数
...
- 新增「车辆登记资料需要更新」业务提示识别（`لا يوجد لهذه الرخصة بيانات مسجلة حديثة`）：App 与插件提供准确中文说明，且不计入官网故障或熔断统计；插件收到官网错误时会隐藏上一笔成功结果，避免误认成当前查询结果。
- 校准现行埃及号牌的 17 字母快捷面板；修正字母 ه 的显示与传参：界面使用号牌上的独立蛋形 `ه`，后台兼容旧配置中的连接形 `هـ` 并统一向 PPO 官网提交单字符 `ه`。
...
- 护照格式最多自动切换一次；若 PPO 明确返回内部会话认证异常，程序会重建隐藏浏览器会话并额外重试一次。两种自愈都有独立硬上限，单次任务最多三次提交，不会循环刷新或无限查询。
- 查询结果、错误诊断、响应耗时和原始快照本地归档；无论成功或失败，都会保留最终一次实际提交前与 PPO 最终页面两张现场截图。普通用户只能鉴权查看当前设备自己的截图，Admin 可查看全部。
...
- 护照格式：首次提交 + 最多 1 次替代格式重试；官网内部会话异常可额外重建并重试 1 次，合计最多 3 次。
...
```text
dist/ppo-traffic-autofill-chrome-v1.0.11.zip
dist/ppo-traffic-autofill-edge-v1.0.11.zip
dist/ppo-traffic-autofill-chrome-latest.zip
dist/ppo-traffic-autofill-edge-latest.zip
```

带版本号的 Chrome、Edge ZIP 分别用于两个商店；`latest` 文件方便本地反复验证。两个浏览器目前都使用同一套 Manifest V3 源码，但使用独立文件名可以避免上架时选错包。正式上传前请确认商店当前已发布版本低于 `1.0.11`；如果商店已经发布过 `1.0.11`，则必须提升版本号后再提交。

## GitHub 自动构建与 Release

仓库内置 `.github/workflows/build-release.yml`：

- 推送到 `main` 时，如果提交尚未升版则自动递增补丁版本；如果本地已经统一升版则直接沿用该版本，保证本地与远端产物一致，并避免重复递增。
- 自动创建同版本 Git 标签，校验并打包 Chrome ZIP、Edge ZIP、macOS Universal DMG/ZIP 和 Ed25519 签名查询核心 ZIP，随后创建 GitHub Release 并附上 5 个可下载文件。
- 手工推送形如 `v1.0.11` 的版本标签时仍会验证并构建该版本，但一般无需再手工创建标签。
...
- `.github/workflows/core-release.yml` 对 Web/API/查询驱动等核心改动建立独立 `core-vX.Y.Z` Release；只生成核心数据包，不重新构建完整 App。App 使用内置公钥验证核心签名、逐文件 SHA-256 和外壳兼容版本后才允许切换，并在健康检查失败时自动回滚。
