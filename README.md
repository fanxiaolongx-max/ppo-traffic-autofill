...
- Admin 可按完整证件号或车牌号配置精确匹配的短信通知规则；查询成功或失败后异步调用外部短信 API，使用固定幂等键防止重复发送，并单独保存投递状态，不阻塞查询队列
- 用户可在查询成功后的结果详情绑定埃及手机号；`+20` 前缀固定，支持输入带或不带本地 `0` 的号码，必须完成短信验证码验证才会生效。验证后默认每 7 天自动查询，可调整为最短 24 小时，并与前台查询共用串行队列及设备 ID + IP 防滥用限额
