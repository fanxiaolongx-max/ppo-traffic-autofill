...
function renderServiceHistory(events) { $('#service-history').innerHTML=events.slice(0,40).map(event=>`<article class="service-event"><i class="${escapeHtml(event.status)}"></i><strong>${escapeHtml(componentLabel(event.component))}</strong><span>${escapeHtml(t(event.message))} <small class="muted">${escapeHtml(event.code||'')}</small></span><time>${formatTime(event.createdAt)}</time></article>`).join('')||`<p class="muted">${t('暂无服务状态记录')}</p>`; }

async function loadOverview() {
  const data=await adminApi('/api/v1/admin/overview'); state.overview=data; renderMetrics(data.status); renderQueue(data.queue); renderServiceHistory(data.status.internalEvents||data.status.events);
...
function renderQueries(data,append=false) {
  ...
  $('#query-total').textContent=getLanguage()==='en'?`${data.total} total`:`共 ${data.total} 条`; $('#load-more-queries').classList.toggle('hidden',!data.hasMore); bindQueryButtons(); bindLogSearchButtons();
}
...
async function loadFeedback({append=false}={}) { if(!append) state.feedbackOffset=0; const params=new URLSearchParams({q:$('#feedback-search').value.trim(),status:$('#feedback-status').value,limit:'50',offset:String(state.feedbackOffset)}); const data=await adminApi(`/api/v1/admin/feedback?${params}`); state.feedback=append?[...state.feedback,...data.items]:data.items; renderFeedback(data,append); }
...
function renderLogs(data) { state.logs=data.items; $('#log-path').textContent=data.logDir; $('#log-path').title=`${data.logDir}\n${data.diagnosticsDir}`; $('#log-body').innerHTML=data.items.map((entry,index)=>`<tr class="log-row" data-log-index="${index}" title="${t('点击查看该条日志的完整单行 JSON')}"><td class="mono">${escapeHtml(entry.localTimestamp||formatTime(entry.timestamp))}</td><td><span class="level ${escapeHtml(entry.level)}">${escapeHtml(String(entry.level||'').toUpperCase())}</span></td><td class="mono">${escapeHtml(entry.event)}</td><td class="mono">${escapeHtml(entry.traceId||'—')}</td><td class="mono">${escapeHtml(entry.plate||'—')}</td><td class="mono">${escapeHtml(entry.documentMasked||'—')}</td><td>${escapeHtml(logSummary(entry))}</td></tr>`).join('')||`<tr><td colspan="7" class="muted">${t('没有匹配的日志')}</td></tr>`; $('#log-summary').textContent=getLanguage()==='en'?`Showing ${data.items.length} · Updated ${formatTime(data.generatedAt)}`:`显示 ${data.items.length} 条 · 更新时间 ${formatTime(data.generatedAt)}`; document.querySelectorAll('.log-row').forEach(row=>row.addEventListener('click',()=>{$('#log-detail').textContent=JSON.stringify(state.logs[Number(row.dataset.logIndex)]);$('#log-dialog').showModal();})); }
async function loadLogs() { const params=new URLSearchParams({q:$('#log-search').value.trim(),level:$('#log-level').value,event:$('#log-event').value.trim(),limit:'500'}); renderLogs(await adminApi(`/api/v1/admin/logs?${params}`)); }
...
$('#refresh-overview').addEventListener('click',loadOverview); $('#query-filter').addEventListener('submit',event=>{event.preventDefault();loadQueries();}); $('#load-more-queries').addEventListener('click',()=>{state.queryOffset+=50;loadQueries({append:true});});
$('#feedback-filter').addEventListener('submit',event=>{event.preventDefault();loadFeedback();}); $('#load-more-feedback').addEventListener('click',()=>{state.feedbackOffset+=50;loadFeedback({append:true});});
$('#log-filter').addEventListener('submit',event=>{event.preventDefault();loadLogs();}); document.querySelectorAll('[data-event]').forEach(button=>button.addEventListener('click',()=>{$('#log-event').value=button.dataset.event;loadLogs();})); document.querySelectorAll('[data-close]').forEach(button=>button.addEventListener('click',()=>document.getElementById(button.dataset.close).close()));
initializePreferences($('.header-actions'));
window.addEventListener('ppo:languagechange',()=>{ if(state.auth)Promise.all([loadOverview(),loadQueries(),loadFeedback(),loadLogs()]).then(()=>applyTranslations()).catch(()=>{}); else showLogin({passwordConfigured:!$('#login-password').disabled}); });
